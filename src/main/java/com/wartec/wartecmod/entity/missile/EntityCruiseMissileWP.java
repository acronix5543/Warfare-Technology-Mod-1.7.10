...
public class EntityCruiseMissileWP extends EntitySubsonicCruiseMissileBase {
	...
	public EntityCruiseMissileWP(World world, float x, float y, float z, int a, int b, TileEntityVlsExhaust exh) {
	...
	public void onImpact() {
		...
		explosionLargeAdvanced.standardExplosion(worldObj, posX, posY, posZ, 50F, 5F, false);
		ExplosionLarge.spawnShrapnels(worldObj, posX, posY, posZ, 55);
		ExplosionChaos.burn(worldObj, (int) posX, (int) posY, (int) posZ, 50);
		int radius = 30;
		List<Entity> hit = worldObj.getEntitiesWithinAABBExcludingEntity(null, AxisAlignedBB.getBoundingBox(posX - radius, posY - radius, posZ - radius, posX + radius, posY + radius, posZ + radius));
		for(Entity e : hit) {
			e.setFire(5);
		...
		for(int i = 0; i < 10; i++) {
			...
			PacketDispatcher.wrapper.sendToAllAround(new AuxParticlePacketNT(haze, posX + worldObj.rand.nextGaussian() * 15, posY, posZ + worldObj.rand.nextGaussian() * 15), new NetworkRegistry.TargetPoint(worldObj.provider.dimensionId, posX, posY, posZ, 150));
		}
