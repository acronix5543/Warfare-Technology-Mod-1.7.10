...
public abstract class EntityHypersonicCruiseMissileBase extends Entity implements IChunkLoader, IRadarDetectable, ITurretTargetable {
	...
	public EntityHypersonicCruiseMissileBase(World world, float x, float y, float z, int a, int b) {
		super(world);
		this.ignoreFrustumCheck = true;
		/*this.posX = x;
		this.posY = y;
	    ...
	    Range = (Math.sqrt(((targetX - startX)*(targetX - startX)) + 0 + ((targetZ - startZ)*(targetZ - startZ))));
		
		transformationpointvector = (Math.sqrt(((targetX - startX)*(targetX - startX)) + 0 + ((targetZ - startZ)*(targetZ - startZ))))*0.15;
